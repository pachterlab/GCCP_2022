...
from tools_conditional import hellinger
# -
...
compare_array_norm_hellinger = np.zeros((7,256,2))
compare_array_hellinger = np.zeros((7,256,2))
...
for i in range(256):
    ...
    compare_array_norm_hellinger[6,i,0] = (t2-t1)/(ss)
    compare_array_norm_hellinger[6,i,1] = hellinger(y_20std,y_NN_new)/(ss)
    compare_array_hellinger[6,i,0] = (t2-t1)
    compare_array_hellinger[6,i,1] = hellinger(y_20std,y_NN_new)
...
NN_hellinger_1NB = np.zeros(256*3)
...
for i in range(256*3):
    ...
    NN_hellinger_1NB[i] = tools.hellinger(predicted_NN_new,pdf)


np.save('../results/NN_hellinger',NN_hellinger)
np.save('../results/NB_hellinger',NB_hellinger)
np.save('../results/NN_hellinger_1NB',NN_hellinger_1NB)
...
NN_times_onepoint_1NB = np.zeros(256*3)
...
for i in range(256*3):
    ...
    t1 = time.time()
    N,M = np.meshgrid(np.arange(1),np.arange(1),indexing='ij')
    y_NN_1NB = nnNB_module_new.nnNB_prob(p_in,
                            n = N,m = M,model = model,use_old=False)
    t2 = time.time()

    NN_times_onepoint_1NB[i] = (t2-t1)

np.save('../results/NN_times_onepoint',NN_times_onepoint)
np.save('../results/NN_times_onepoint_1NB',NN_times_onepoint_1NB)
